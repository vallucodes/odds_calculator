...
RUN_NOTE = "logistic_combined_l15_>10matches"
...
rolling_df = pd.read_parquet(f"{FEATURES_DIR}/features_rolling_l15.parquet")[[
    'hltv_match_id',
    'team1_rolling_rating_l15','team2_rolling_rating_l15',
    'team1_rolling_kast_l15','team2_rolling_kast_l15',
    'team1_rolling_swing_l15','team2_rolling_swing_l15',
    'team1_rolling_win_rate_l15','team2_rolling_win_rate_l15'
]]

rolling_df['rating_diff_l15'] = rolling_df['team1_rolling_rating_l15'] - rolling_df['team2_rolling_rating_l15']
rolling_df['kast_diff_l15'] = rolling_df['team1_rolling_kast_l15'] - rolling_df['team2_rolling_kast_l15']
rolling_df['swing_diff_l15'] = rolling_df['team1_rolling_swing_l15'] - rolling_df['team2_rolling_swing_l15']

df = df.merge(rolling_df, on='hltv_match_id', how='left')

map_df = pd.read_parquet(f"{FEATURES_DIR}/features_map_winrate_l15.parquet")

wr_cols = [c for c in map_df.columns if '_wr_l15' in c]
map_df[wr_cols] = map_df[wr_cols].fillna(0.5)

maps = ['ancient', 'anubis', 'dust2', 'inferno', 'mirage', 'nuke', 'overpass', 'train', 'vertigo']

for m in maps:
    map_df[f'{m}_wr_diff_l15'] = map_df[f'team1_{m}_wr_l15'] - map_df[f'team2_{m}_wr_l15']

df = df.merge(map_df[['hltv_match_id'] + [f'{m}_wr_diff_l15' for m in maps]], on='hltv_match_id', how='left')
...
feature_cols = [
    'elo_diff', 'rating_diff_l15', 'kast_diff_l15', 'swing_diff_l15',
    'team1_rolling_win_rate_l15', 'team2_rolling_win_rate_l15', 'is_lan'
] + [f'{m}_wr_diff_l15' for m in maps]
