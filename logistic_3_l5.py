...
rolling_df = pd.read_parquet(f"{FEATURES_DIR}/features_rolling_l5.parquet")[[
    ...
    'team1_rolling_win_rate_l5','team2_rolling_win_rate_l5'
]]
rolling_df['rating_diff_l5'] = rolling_df['team1_rolling_rating_l5'] - rolling_df['team2_rolling_rating_l5']
rolling_df['kast_diff_l5'] = rolling_df['team1_rolling_kast_l5'] - rolling_df['team2_rolling_kast_l5']
rolling_df['swing_diff_l5'] = rolling_df['team1_rolling_swing_l5'] - rolling_df['team2_rolling_swing_l5']
df = df.merge(rolling_df, on='hltv_match_id', how='left')

map_df = pd.read_parquet(f"{FEATURES_DIR}/features_map_winrate_l5.parquet")
wr_cols = [c for c in map_df.columns if '_wr_l5' in c]
map_df[wr_cols] = map_df[wr_cols].fillna(0.5)
maps = ['ancient', 'anubis', 'dust2', 'inferno', 'mirage', 'nuke', 'overpass', 'train', 'vertigo']
for m in maps:
    map_df[f'{m}_wr_diff_l5'] = map_df[f'team1_{m}_wr_l5'] - map_df[f'team2_{m}_wr_l5']
df = df.merge(map_df[['hltv_match_id'] + [f'{m}_wr_diff_l5' for m in maps]], on='hltv_match_id', how='left')

filtered_df = df[
    (df['team1_games'] > 30) &
    (df['team2_games'] > 30) &
    (df['team1_rolling_rating_l5'].notna()) &
